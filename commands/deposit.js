...
module.exports = {
    name: 'deposit',
    description: 'take money from your wallet to your bank \nex: $deposit 50100',
    execute(msg, args, utils) {
        if (!args[1]) {
            msg.channel.send("Please enter the ammount you want to deposit")
            return
        }
            ...
            .then(res => {
                ...
                for (let i = 0; i < data1.userData.length; i++) {
                    if (data1.userData[i].id === msg.author.id) {
                        if (parseInt(args[1]) > data1.userData[i].money.wallet) {
                            msg.channel.send("You do not have this ammount in your wallet")
                            return
                        }
                        ...
                        utils.send(data1)
                            .then(res => {
                                msg.channel.send("Successfully transferred $" + args[1] + " to your bank")
